...
describe('Chess PGN Parser', () => {
  ...
  test('parse FEN with en passant', () => {
    ...
    const parser = new PGNParser(pgn);
    expect(parser.haveFEN).toBe(true);
    expect(parser.getRoot().fen).toBe('rnbqkbnr/1ppppppp/p7/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 2');
  });
  ...
  test('parse FEN with no castling available', () => {
    ...
    const parser = new PGNParser(pgn);
    expect(parser.haveFEN).toBe(true);
    expect(parser.getRoot().fen).toBe('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w - - 0 1');
  });
  ...
  test('default fen when no FEN tag provided', () => {
    ...
    expect(parser.getRoot().fen).toBe('rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1');
  });
